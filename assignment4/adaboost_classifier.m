...
function [classifier, alpha_list, alpha_threshold] = adaboost_classifier(train_data, train_label, current_label, haar_list)
    ...
    classifier = {};
    weights = zeros(size(train_data, 1), 1); %generates the haar matrix and wieghts
    ...
    for i = 1:num_iterations
        ...
        classifier{i} = haar_list{error_index}; %finds the optimal haar feature to use
        ...
        beta = min_error / (1 - min_error); %finds the error that the needed
        ...
        weights = weights .* exp((0.5 * alpha) * invert_prediction); %reupdates the weights
    end
...
function [min_error, error_index, error_haar_threshold, error_parity] = find_optimal_haar(haar_matrix, weights, binary_labels)
    ...
    for i = 1:num_haar_features
        %disp(i);
        for j = 1:(num_thresholds-1) %test al of them
            haar_threshold = haar_matrix(j, i);
            bin_matrix = haar_matrix(:, i) > haar_threshold;
            error = sum(weights .* abs(bin_matrix - binary_labels)); %find error
            parity = 1;
